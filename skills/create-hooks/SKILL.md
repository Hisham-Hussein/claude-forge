...
**Inject context on each prompt (UserPromptSubmit)**:
```json
{
  "hooks": {
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "echo 'Remember: Always run tests before committing'"
          }
        ]
      }
    ]
...
**Important:** UserPromptSubmit uses **plain stdout** for context injection, NOT JSON fields:
- ✅ Plain text via `echo` or `print()` → injected as system reminder
- ❌ `{"message": "..."}` → ignored
- ❌ `{"additionalContext": "..."}` → ignored
- ❌ `{"hookSpecificOutput": {...}}` → ignored

This differs from SessionStart which uses JSON `hookSpecificOutput.additionalContext`.
</common_patterns>
